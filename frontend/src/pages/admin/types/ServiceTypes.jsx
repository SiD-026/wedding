...
const ServiceTypes = () => {
  ...
  const getTypes = async () => {
    return Typesapi.getAllServiceType().then((res) => setTypes(res.data));
  };
  ...
  const handleClick = () => {
    navigate("/admin/dashboard/serviceType/add");
  };
  ...
  const handleSearch = async (e) => {
    ...
    if (key) {
      const result = await Typesapi.searchTypes({ type: key });
  ...
  const handleDelete = async () => {
    try {
      await Typesapi.deleteServiceTypeById(deleteId);
      setTypes(types.filter((data) => data._id !== deleteId));
      setModal(false);
      Toast.deleted();
    } catch (error) {
      Toast.error(error);
  ...
  const currentRecords =types.length > 0 ? types.slice(indexOfFirstRecord, indexOfLastRecord): ""
  const nPages = types ? Math.ceil(types.length / recordsPerPage) : 0
        ...
        <div className="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
          <div className="flex py-2 pt-10 pb-5 px-5">
            <div className="bg-indigo-600 px-2 rounded-lg mr-10">
              <h2 className="font-medium text-3xl text-white">Service Types</h2>
            </div>
            <input
                  ...
                  currentRecords.map((type, index) => (
                    <tr key={type._id}>
                      ...
                      <td className="text-sm font-medium flex pt-4">
                        <NavLink
                          to={`/admin/dashboard/serviceType/edit/${type._id}`}
                          className="mr-3"
                        >
                        ...
                        <label
                          htmlFor="modal-2"
                          onClick={() => handleDeleteById(type._id)}
                        >
