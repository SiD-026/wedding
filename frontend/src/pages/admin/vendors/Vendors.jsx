...
const Vendors = () => {
  ...
  const getVendors = async () => {
    const res = await vendorapi.searchVendors({ sort: "date", by: "desc" });
  ...
  const handleClick = () => {
    navigate("/admin/dashboard/vendors/add");
  };
  ...
  const handleSearch = async (e) => {
    ...
    if (key) {
      const result = await vendorapi.searchVendors({ name: key });
  ...
  const handleDelete = async () => {
    try {
      await vendorapi.deleteVendorById(deleteId);
      setVendors(vendors.filter((data) => data._id !== deleteId));
      setModal(false);
      Toast.deleted();
    } catch (error) {
      Toast.error(error);
  ...
  const currentRecords = vendors.slice(indexOfFirstRecord, indexOfLastRecord);
        ...
        <div className="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
          <div className="flex py-2 pt-10 pb-5 px-5">
            <div className="bg-indigo-600 px-2 rounded-lg mr-10">
              <h2 className="font-medium text-3xl text-white">Vendor</h2>
            </div>
            <input
            ...
            <table className="w-full text-center">
              ...
              <tbody className="bg-white divide-y divide-gray-200 ">
                {loading ? (
                  ...
                  currentRecords.map((vendor, index) => (
                    <tr key={vendor._id}>
                      ...
                      <td>
                        <div className="text-md text-gray-800 font-medium ">
                          {moment(vendor.joinedAt).utc().format("YYYY/MM/DD")}
                        </div>
                      </td>
                      <td className="text-sm font-medium flex pt-3">
                        <NavLink
                          to={`/admin/dashboard/vendors/edit/${vendor._id}`}
                          className="mr-3"
                        >
                        ...
                        <label
                          htmlFor="modal-2"
                          onClick={() => handleDeleteById(vendor._id)}
                        >
